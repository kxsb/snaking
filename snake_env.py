...
class SimpleSnakeEnv:
    def __init__(self, grid_size=(20, 20), cell_size=20):
        self.grid_size = grid_size
        self.cell_size = cell_size
        self.width = self.grid_size[0] * self.cell_size
        self.height = self.grid_size[1] * self.cell_size
        self.snake = [(self.cell_size * 5, self.cell_size * 5), 
                      (self.cell_size * 4, self.cell_size * 5), 
                      (self.cell_size * 3, self.cell_size * 5)]
    ...
    def check_grid_size(self, model_grid_size):
        """Vérifie si la taille de la grille correspond à une taille supportée."""
        return self.grid_size == model_grid_size
    ...
    def spawn_food(self):
        while True:
            x = random.randint(0, self.grid_size[0] - 1) * self.cell_size
            y = random.randint(0, self.grid_size[1] - 1) * self.cell_size
            if (x, y) not in self.snake:
                return x, y
